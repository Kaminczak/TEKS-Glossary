...
function SidebarSubject({ subject, activeSubject, expanded, onToggle, onNavigate, activeStrand }) {
  ...
  if (subject.kind === 'link') {
    return (
      <button
        className="relative flex items-center gap-2.5 px-3 py-2 rounded-md text-sm font-medium text-left transition-colors"
        style={{
          color: PALETTE.inkSecondary,
          ...
          {aiOpen && (
            <div className="ml-4 my-0.5 flex flex-col">
              {AI_TOOL_NAMES.map((toolName, i) => {
                // First two ("Focus Package", "Warm-Ups") = available; rest = "coming soon"
                const available = i < 2;
                return (
                  <button
                    key={toolName}
